...
snakemake <- Snakemake(
    input = list('/shares/CIBIO-Storage/BCG/scratch/fgastaldello/resources/protein_coding_transcripts/wt_cds.RData', '/shares/CIBIO-Storage/BCG/scratch/fgastaldello/resources/protein_coding_transcripts/protein_coding_transcripts.RData', '/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/temp/UVM_list.txt', '/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/tumors/UVM/phased_vcf/chr11.vcf.gz', '/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/tumors/UVM/phased_vcf/chr11.vcf.gz.tbi', "wt_cds" = '/shares/CIBIO-Storage/BCG/scratch/fgastaldello/resources/protein_coding_transcripts/wt_cds.RData', "annotations" = '/shares/CIBIO-Storage/BCG/scratch/fgastaldello/resources/protein_coding_transcripts/protein_coding_transcripts.RData', "samples_list" = c('/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/temp/UVM_list.txt'), "vcf" = c('/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/tumors/UVM/phased_vcf/chr11.vcf.gz'), "index" = c('/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/tumors/UVM/phased_vcf/chr11.vcf.gz.tbi')),
    output = list('/shares/CIBIO-Storage/BCG/scratch/fgastaldello/data/cvep-plm/tumors/UVM/mutated_sequences/nn/chr11.RData'),
    params = list(),
    wildcards = list('UVM', '11', "cancer_type" = 'UVM', "chr" = '11'),
    threads = 1,
    log = list(),
    resources = list(),
...
load(snakemake@input[["samples_list"]])
...
for (sample in sample_list) {
        for (transcript in unique(protein_coding_transcripts[which(protein_coding_transcripts$chromosome_name==chr),"ensembl_transcript_id"])) {
...
save(mutated_sequences, file = paste0("/shares/CIBIO-Storage/BCG/scratch/proteinModel/phasing/resources/cds/mutated_nucleotide_sequences/mutated_sequences_chr", chr ,".RData"))
cat(paste("Done chromosome", chr,". Total elapsed time:", round(seconds_to_period(duration), 2), "\n"))


# Need to recompose the mutated_sequences dataframe after splitting it for chromosome
chr <- 1
load(file = paste0("/shares/CIBIO-Storage/BCG/scratch/proteinModel/phasing/resources/cds/mutated_sequences_chr", chr ,".RData"))
temp <- mutated_sequences

for (chr in seq(2,22,1)) {
        
        load(file = paste0("/shares/CIBIO-Storage/BCG/scratch/proteinModel/phasing/resources/cds/mutated_sequences_chr", chr ,".RData"))
        mutated_sequences <- mutated_sequences %>% drop_na()
        temp <- rbind(temp, mutated_sequences)
        
}

mutated_sequences <- temp; rm(temp)
save(mutated_sequences, file = "/shares/CIBIO-Storage/BCG/scratch/proteinModel/phasing/resources/cds/mutated_nucleotide_sequences.RData")
...
which_rows <- function(transcripts_anno, variants_coord, chr) {
  ...
  for (transcript in unique(transcripts_anno[which(transcripts_anno$chromosome_name==chr), "ensembl_transcript_id"])) {
    
    exons <- protein_coding_transcripts[which(protein_coding_transcripts$ensembl_transcript_id==transcript),]
    ...
    for (exon in 1:nrow(exons)) {
      ...
      IDs <- c(IDs,variants_coord[which(variants_coord$CHROM==chr &
                                      variants_coord$POS>=start &

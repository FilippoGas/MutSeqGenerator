...
# load transcript sequences and annotations
...
load("/shares/CIBIO-Storage/BCG/scratch/proteinModel/phasing/resources/cds/sample_list.RData")
...
foreach (chr = seq(1,22,1), .packages = c("vcfR", "tidyverse")) %dopar% {
  
  # generate path and load vcf
  path <- paste0("/shares/CIBIO-Storage/BCG/scratch/proteinModel/phasing/phased_genotypes/final_phased/vcf_files/TCGA-BRCA1-chr", chr, "-phased.vcf.gz")
  vcf <- read.vcfR(file = path)
